...
function AdminsSection({ admins }) {
  return (
    <HiveCard>
      <CardHeader icon="👤" title="Admins" count={admins?.length ?? 0} />
      <CardBody>
        {!admins?.length ? <EmptyState msg="No admin records" /> : (
            ...
            {admins.map(a => (
              <div key={a.admin_id} style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                  <div style={{
                    width: 38, height: 38, borderRadius: 10,
                    background: T.dark, display: "flex",
                    alignItems: "center", justifyContent: "center",
                    fontSize: 16, flexShrink: 0,
                  }}>👤</div>
                  <div>
                    <div style={{ fontWeight: 800, color: T.text, fontSize: 14 }}>{fmt(a.name)}</div>
                    <div style={{ color: T.accent, fontSize: 12, fontWeight: 600 }}>{fmt(a.email)}</div>
                  </div>
                </div>
                <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: 10, display: "flex", flexDirection: "column", gap: 4 }}>
                  <div style={{ color: T.textMuted, fontSize: 12, display: "flex", alignItems: "center", gap: 5 }}>
                    <span style={{ fontSize: 13 }}>🏢</span> {fmt(a.department)}
                  </div>
                  <div style={{ color: T.textMuted, fontSize: 12, display: "flex", alignItems: "center", gap: 5 }}>
                    <span style={{ fontSize: 13 }}>📞</span> {fmt(a.phone)}
                  </div>
                </div>
              </div>
            ))}
